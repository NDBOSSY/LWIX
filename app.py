...
class User(UserMixin, db.Model):
    ...
    def get_membership_duration_display(self):
        ...
        if self.subscription_duration_days >= 365:
            return f"{self.subscription_duration_days / 365:.0f} Year"
        if self.subscription_duration_days >= 30:
            return f"{self.subscription_duration_days / 30:.0f} Month"
...
def user_login():
    ...
    if request.method == "POST":
        ...
        if not user.email_verified:
            flash("Account not active. Complete purchase on Wix first.", "error")
            return render_template("user/login.html")
...
def user_dashboard():
    ...
    return render_template(
        ...
        license_accounts=license_accounts, account_count=account_count,
...
def generate_license():
    ...
    try:
        ...
        days = current_user.subscription_duration_days or Config.LICENSE_EXPIRY_DAYS
        lic = License(
        ...
        logger.error(f"License failed: {e}")
        db.session.rollback()
...
def reset_license_account(account_id):
    ...
    if account:
        ...
        db.session.commit()
        flash("Account slot freed.", "success")
...
def api_validate_license():
    try:
        ...
        existing = LicenseAccount.query.filter_by(
            license_id=lic.id, account_number=account_id
        ).first()
        ...
        if active_count >= lic.max_accounts:
            return jsonify({
                "valid": False,
                "error": f"Maximum {lic.max_accounts} accounts reached. Contact support to upgrade.",
...
def assign_discord_role(discord_id):
    try:
        ...
        logger.error(f"Discord role failed: {e}")
        return False
