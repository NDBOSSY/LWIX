...
def log_audit(user_id: int, action: str, details: str = None, ip_address: str = None):
    try:
        log = AuditLog(user_id=user_id, action=action, details=details, ip_address=ip_address or (request.remote_addr if request else "system"))
...
def user_login():
    ...
    if request.method == "POST":
        ...
        except EmailNotValidError: flash("Invalid email.", "error"); return render_template("user/login.html")
        ...
        if not user.email_verified: flash("Account not active. Complete purchase on Wix first.", "error"); return render_template("user/login.html")
        ...
        try:
            ...
            send_email_async("Your OTP - Trading Engine", [email], f"OTP: {otp}",
...
@login_required
def logout():
    log_audit(current_user.id, "logout", request.remote_addr); logout_user(); flash("Logged out.", "success")
    return redirect(url_for("user_login"))
...
def delete_ea(ea_id):
    ...
    if ea:
        ...
        log_audit(current_user.id, "ea_deleted", name, request.remote_addr); flash(f"'{name}' deleted.", "success")
    return redirect(url_for("admin_dashboard"))
...
def api_validate_license():
    try:
        ...
        if not lic: return jsonify({"valid": False, "error": "Invalid"}), 404
        if not lic.is_valid(): return jsonify({"valid": False, "error": "Not active"}), 403
        if lic.machine_id and lic.machine_id != data.get("machine_id", ""): return jsonify({"valid": False, "error": "Machine mismatch"}), 403
        if not lic.machine_id and data.get("machine_id"): lic.machine_id = encrypt_data(data["machine_id"])
    ...
    except Exception as e: logger.error(f"Validation failed: {e}"); return jsonify({"valid": False}), 500
...
def wix_payment_webhook():
    try:
        ...
        send_email_async("Welcome to Trading Engine! 🎉", [email],
                         ...
                         f"<h3>Hi {user_name}!</h3><p>Plan: {plan_name}</p><p>Login: {Config.APP_URL}/login</p>")
        log_audit(user.id, "wix_plan_ordered", f"{'New' if is_new else 'Updated'} | {plan_name} | {subscription_type}", request.remote_addr)
        logger.info(f"✅ Plan: {email} | {plan_name} | {subscription_type}")
